...
public class StringAlgorithmsDemo
{
	public static void main(String[] args)
	{
		...
		printTwoCharactersAtATime(name);
		System.out.println();
		System.out.println();
	...
	// Accesses two characters of a string two by two and prints each set of two
	// characters on a separate line.  The algorithm only advances one character
	// during each iteration of the loop because i is just incremented by one
	// with i++ in the loop header.
	public static void printTwoCharactersAtATime (String str)
	{
		for (int i = 0; i < str.length(); i++)
		{
			String twoLetters = str.substring(i, i+2);
			//System.out.println("The characters beginning at position " + i + " are " + twoLetters);
