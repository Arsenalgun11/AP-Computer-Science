...
public class SubstringMania2Solutions
{
	public static void main(String[] args)
	{
		Scanner reader = new Scanner (System.in);	
	
		// Part 1. STOP AND WRITE THE deleteCharacter METHOD AT THE END OF THIS FILE.
		...
		// Part 2. Write the code that will count the number of words in a sentence,
		...
		for(int i = 0; i < sentence.length(); i++)
		{
			...
			if(letter.compareTo(" ") == 0 || letter.compareTo(".") == 0
					|| letter.compareTo("?") == 0 || letter.compareTo("!") == 0)
			{
				wordCount++;
			}
		}
		...
		{
			int posOfBlank = sentence.indexOf(" ");
			int posOfPeriod = sentence.indexOf(".");
			int posOfQMark = sentence.indexOf("?"); 
			int posOfExclam = sentence.indexOf("!"); 
			if (posOfBlank != -1)
			{
				// cut off the word before the blank and retrieve the remainder of sentence
				sentence = sentence.substring(posOfBlank + 1);
			}
			else if (posOfPeriod != -1  || posOfQMark != -1  ||  posOfExclam != -1)
			{
				// at end of sentence so give the variable sentence empty string
				sentence = "";
			}
			wordCount++;
		}
